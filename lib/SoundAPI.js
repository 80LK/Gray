 ...
 */
/*ChangeLog:
	v2.4 Multiplayer update
		- Add method Sound.registerNetworkFile(sid, path)
		- Add method Sound.getNetworkFile(sid)
		- Add method Sound.registerTileEntity(id, Prototype)
	v2.3
		- Fix shared.
...
var _soundUtils = {
	...
	updateVolume:function(){
		for(var i in this.sounds){
			...
			//Game.tipMessage("ID: " + s._id + "\nX:" + parseInt(pp.x) + " Y:" + parseInt(pp.y) + " Z:" + parseInt(pp.z) + "\n" + left_volume + " : " + right_volume + " : " + gameVolume);
...
Sound.registerNetworkFile = function(sid, file){
    if(_soundUtils.networkFiles.hasOwnProperty(sid))
        throw new Error("sid файла \""+sid+"\" уже существует");

    _soundUtils.networkFiles[sid] = file;
    return sid;
};
Sound.getNetworkFile = function(sid){
    if(!_soundUtils.networkFiles.hasOwnProperty(sid))
        throw new Error("sid файла \""+sid+"\" не найден");

    return _soundUtils.networkFiles[sid];
};
Sound.registerTileEntity = function(BlockID, Prototype){
    if(!Prototype) Prototype = {};
	if(!Prototype.events) Prototype.events = {};
	if(!Prototype.defaultValues) Prototype.defaultValues = {};

	if(!Prototype.client) Prototype.client = {}
	if(!Prototype.client.events) Prototype.client.events = {};
    
	//Server
	Prototype.defaultValues.SoundAPI = {
		source:null,
		start:0,
		playing:false
	};

	Prototype._init = Prototype.init || function(){};
	Prototype.init = function(){
		if(!this.data.SoundAPI)
			this.data.SoundAPI = { source:null, start:0, playing:false };

		this._init();
	}

	Prototype.Play = function(){
		this.data.SoundAPI.start = System.currentTimeMillis();
		this.data.SoundAPI.playing = true;

		this.sendPacket("SoundAPI_Play", this.data.SoundAPI);
	}
	Prototype.Stop = function(preparing){
		if(!preparing) preparing = false;
		this.data.SoundAPI.playing = false;

		this.sendPacket("SoundAPI_Stop", {preparing:preparing});
		this.OnCompletion();
	}
	Prototype.SetSource = function(sid){
		this.data.SoundAPI.source = sid;

		this.sendPacket("SoundAPI_SetSource", {source:sid});
	}

	if(!Prototype.OnCompletion) Prototype.OnCompletion = function(){};
	Prototype.events.SoundAPI_OnCompletion = function(){
		this.OnCompletion();
	}

	Prototype.events.SoundAPI_Init = function(){
		this.sendResponse("SoundAPI_Init", this.data.SoundAPI)
	}
	
	//Client
	Prototype.client._load = Prototype.client.load || function(){};
	Prototype.client.load = function(){
		this.__soundPlayer = new Sound();
		this.__soundPlayer.setInBlock(this.x, this.y, this.z, 16);
		this.__soundPlayer.setOnCompletion((function(){
			this.sendPacket("SoundAPI_OnCompletion");
		}).bind(this));
		alert("ID: " + this.__soundPlayer._id);
		this.sendPacket("SoundAPI_Init");

		this._load();
	}

	Prototype.client._unload = Prototype.client.unload || function(){};
	...
	Prototype.client.events.SoundAPI_Play = function(data){
		if(this.__soundPlayer){
			let millis = System.currentTimeMillis();
			this.__soundPlayer.media.seekTo(millis - data.start);
			this.__soundPlayer.play();
		}
	}

	Prototype.client.events.SoundAPI_Stop = function(data){
		if(this.__soundPlayer)
			this.__soundPlayer.stop(data.preparing);
	}

	Prototype.client.events.SoundAPI_SetSource = function(data){
		if(this.__soundPlayer)
			this.__soundPlayer.setSource(Sound.getNetworkFile(data.source));
	}

	Prototype.client.events.SoundAPI_Init = function(data){
		let sid = data.source;
		if(sid != null)
			this.__soundPlayer.setSource(Sound.getNetworkFile(sid));
